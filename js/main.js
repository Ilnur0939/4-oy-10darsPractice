...
let filterSortby = document.querySelector('#sortby')
studentFilter.addEventListener('submit', function(evt){
    ...
    students.filter(item => {
        if(item.name.includes(evt.target[0].value) && evt.target[1].value <= (item.mark * PERCENT / TOTAL_MARK) &&  evt.target[2].value >= (item.mark * PERCENT / TOTAL_MARK)){
    ...
    filterSortby.addEventListener('change', function(e){
        console.log(sortby.value);
        if(sortby.value == 1){
            item.name = item.mark.sort((a,b) => a.name.localeCompare(b.name))
        }
        else if(sortby.value == 2){
            item.mark = item.mark.sort((a,b) => b.mark - a.mark)
        }
    })
